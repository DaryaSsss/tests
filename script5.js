const Passwords = ["nyot babgr babgr kqtu kqtu kzshonp ylyk psqk", 
...
const CheckPassword = () => {
  ...
  for (i = 0; i < Passwords.length; i++) {
    PasswordsArray = Passwords[i].split(" ");
    let PasswordsArray3 = [];
    count = 0;
    SortedLine(PasswordsArray, PasswordsArray3);
    for (m = 0; m < PasswordsArray3.length; m++) {
      if (PasswordsArray3.includes(PasswordsArray3[m], m + 1)) {count++};
    }
    if (count === 0) { result++ }
  }  
  console.log(result)
...
const SortedLine = (PasswordsArray, PasswordsArray3) => {
  for (j = 0; j < PasswordsArray.length; j++) {
    PasswordsArrayLetters = PasswordsArray[j].split("");
    sortPasswordsArray = PasswordsArrayLetters.sort();
    PasswordWord = sortPasswordsArray.join("");
    PasswordsArray3.push(PasswordWord)
  }
}
